...
class BruteForcing(object):
	...
	def actTestConnection(self):
		...
		try:
			...
			self.formLoginID, self.formUsernameField, self.formPasswordField = actions.getFormInformation(process.forms())
	...
	def actTryTargetLogin(self, objBrowser, tryUsername, tryPassword, currentTry):
		try:
			...
			if not actions.getFormInformation(objBrowser.forms()):
